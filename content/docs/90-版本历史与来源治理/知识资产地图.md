...
| UI Pattern | `docs/70-产品维护知识`、`metadata/ui-patterns.csv`、`metadata/ui-style-tokens.csv` | 约束需求 Demo、页面原型和交互样例的布局与视觉一致性 | 只记录共享规则、页面模式、token 和验收清单，不依赖本地 prototype 路径 |
| Prototype Registry | `metadata/prototype-assets.json` + Demo 目录中的 `.linkincrease-prototype.json` | 把需求、摹客产品端入口和分散 HTML Demo 自动关联到需求索引页 | 共享配置不保存个人绝对路径；本地 Demo 通过就近清单声明入口和需求标识 |
...
| 这个需求要做 Demo | UI Pattern + Skill + Wiki | 从空白页随机生成界面，或依赖维护者本地 prototype 文件 |
...
| Demo Prototype | `docs/70-产品维护知识/需求Demo输出规范.md`、`docs/70-产品维护知识/Demo视觉与页面规范.md`、`metadata/ui-patterns.csv`、`metadata/ui-style-tokens.csv`、`skills/linkincrease-demo-prototype` | 生成或评审符合 Linkincrease 页面模式的 Demo、HTML 原型和交互样例 |
