...
| 需求 Demo / 原型 | `skills/linkincrease-demo-prototype`、`docs/70/需求Demo输出规范.md` | 符合 Linkincrease 页面模式的 Demo、HTML 原型或交互样例 |
...
```text
使用 linkincrease-demo-prototype，基于知识库页面模式和视觉 token 生成 Demo：
{需求描述}

请先判断产品端、页面类型和业务对象，选择最接近的 UI pattern；Demo 不依赖本地 prototype 路径。
```
...
5. 只有明确要求 Demo、HTML 原型或交互样例时，才调用 demo prototype workflow；普通 PRD 不展开视觉细节。
6. AI 输出不能直接当结论，必须回到权威知识页、来源和待确认问题做校验。
